...
void UUID::generateUuid()
{
    try
    {
        ...
        std::generate(std::begin(seedData), std::end(seedData), std::ref(randD));
    ...
    {
        throw UUIDException("Error while generating UUID : " + std::string(e.what()));
    }
}
...
void UUID::setUuidFromString(const std::string& uuid)
{
    ...
    {
        throw UUIDException("Error while setting UUID from string : " + std::string(e.what()));
    }
}
