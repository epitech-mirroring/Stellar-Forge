...
Button::Button(IObject *owner, const float rectX, const float rectY, const float width,
               const float height, std::string buttonId)
    : AComponent(owner, new Meta(this)), _rectX(rectX), _rectY(rectY), _width(width),
      _height(height), _buttonId(std::move(buttonId)) {
    EventSystem::getInstance().registerListener("mouse_pressed",
                                                [this](const EventData &eventData) {
                                                    this->onPressed(eventData);
                                                });
    EventSystem::getInstance().registerListener("mouse_released",
                                                [this](const EventData &eventData) {
...
Button *Button::clone(IObject *owner) const {
    return new Button(owner, this->_rectX, this->_rectY, this->_width, this->_height,
                      this->_buttonId);
}

bool Button::inButton(const sf::Event::MouseButtonEvent *mousePos) const {
    return static_cast<float>(mousePos->x) >= this->_rectX && static_cast<float>(mousePos
               ->x) <= this->_rectX + this->_width &&
           static_cast<float>(mousePos->y) >= this->_rectY && static_cast<float>(mousePos
               ->y) <= this->_rectY + this->_height;
}


void Button::onPressed(const EventData &eventData) const {
    if (inButton(static_cast<sf::Event::MouseButtonEvent *>(eventData.data))) {
        EventSystem::getInstance().triggerEvents("button_" + this->_buttonId + "_pressed",
                                                 nullptr);
    }
}
...
void Button::onReleased(const EventData &eventData) const {
    if (inButton(static_cast<sf::Event::MouseButtonEvent *>(eventData.data))) {
        EventSystem::getInstance().triggerEvents(
            "button_" + this->_buttonId + "_released", nullptr);
    }
}
