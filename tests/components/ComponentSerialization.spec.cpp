...
TEST(ComponentSerialization, Transform) {
    ...
    ASSERT_TRUE(data->contains("position"));
    ASSERT_TRUE(data->contains("rotation"));
    ASSERT_TRUE(data->contains("scale"));
    const auto *const position = data->getValue<json::JsonObject>("position");
    const auto *const rotation = data->getValue<json::JsonObject>("rotation");
    const auto *const scale = data->getValue<json::JsonObject>("scale");
